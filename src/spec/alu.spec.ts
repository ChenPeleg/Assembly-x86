...
describe("ALU", () => {
  ...
  it("Set flags after inc and dec", () => {
    let zero: boolean = cpu.statusWord.zero;

    alu.inc(-1);
    expect(cpu.statusWord.zero).toEqual(true);
  ...
  it("Correctly handles signed division", () => {
    ...
    expect(alu.idivide(1, 1)).toEqual(
      joc({
        value: 1,
        remainder: 0,
      })
    ...
    expect(alu.idivide(-50, 2)).toEqual(
      joc({
        value: -25,
        remainder: 0,
      })
    ...
    expect(alu.idivide(80, 3)).toEqual(
      joc({
        value: 26,
        remainder: 2,
      })
    ...
    expect(alu.idivide(Math.pow(2, 33), 4)).toEqual(
      joc({
        value: 0,
        remainder: 0,
      })
  ...
  it("Correctly handles signed multiplication", () => {
    ...
    expect(alu.imultiply(0, 0)).toEqual(
      joc({
        lowerHalf: 0,
        upperHalf: 0,
      })
    ...
    expect(alu.imultiply(5687684, 0)).toEqual(
      joc({
        lowerHalf: 0,
        upperHalf: 0,
      })
    ...
    expect(alu.imultiply(68731, 1)).toEqual(
      joc({
        lowerHalf: 68731,
        upperHalf: 0,
      })
    ...
    expect(alu.imultiply(8879876, -1)).toEqual(
      joc({
        lowerHalf: -8879876,
        upperHalf: 0,
      })
    ...
    expect(alu.imultiply(568765, 2287)).toEqual(
      joc({
    ...
    expect(alu.imultiply(Math.pow(2, 30), 4)).toEqual(
      joc({
        lowerHalf: 0,
        upperHalf: 1,
      })
